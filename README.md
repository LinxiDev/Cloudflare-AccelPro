# Cloudflare-AccelPro
基于 Cloudflare Workers 的 GitHub 和 Docker 加速服务，自动生成加速链接与命令,支持Git协议(目前仅Github)。
# Cloudflare-AccelPro
...
2. **配置构建设置**：
   - 项目名称：输入自定义名称（如 `Cloudflare-AccelPro`）。
...
5. **Git全局配置(Github)**：
   除了提供网页界面和 Docker 加速外，Cloudflare-AccelPro 还支持通过配置 Git 全局设置来加速 GitHub 的 `git clone`、`git fetch` 等操作。
   目前仅做了Github的支持：`https://your-domain.com/gh`
   ```bash
   git config --global url."https://your-domain.com/gh/https://github.com/".insteadOf "https://github.com/"
   ```
   - 最终的.gitconfig如下：
     ```.gitconfig
     # Github 全局代理-Cloudflare-URL重写替换规则
