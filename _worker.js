...
<body class="light-mode">
  ...
  <div class="container mx-auto">
    ...
    <footer class="mt-6 text-center text-gray-500 dark:text-gray-400">
      Powered by <a href="https://github.com/linxidev/Cloudflare-AccelPro" class="text-blue-500 hover:underline">LinxiDev/Cloudflare-AccelPro</a>
    </footer>
  </div>
...
async function handleRequest(request, redirectCount = 0) {
  ...
  if (fullPath.startsWith('https://') || fullPath.startsWith('http://')) {
    ...
    // 如果目标是 github.com 的 HTTP(s) 路径，则视为 Git 请求
    if (targetDomain === 'github.com') {
      isGitRequest = true;
    }
  } else {
    // 处理 Docker 镜像路径的多种格式
    // 支持 /gh 前缀用于 GitHub 的 Git over HTTP(S) 代理
    if (pathParts[0] === 'gh') {
      // /gh/owner/repo(.git)/... -> 代理到 github.com
      isGitRequest = true;
      ...
      // 若目标为 github.com 且路径包含 Git HTTP endpoints，则视为 Git 请求
      if (targetDomain === 'github.com') {
        const lowerPath = targetPath.toLowerCase();
        if (lowerPath.includes('/info/refs') || lowerPath.includes('/git-upload-pack') || lowerPath.includes('/git-receive-pack') || lowerPath.includes('/info/lfs')) {
          isGitRequest = true;
        }
      }
  ...
  // Git 请求需要保留或设置 Git 专用头
  if (isGitRequest) {
    // 保证 User-Agent 为 Git 客户端风格，如果客户端未设置或不是 git/ 前缀则设置默认
    const ua = newRequestHeaders.get('user-agent') || '';
    if (!/\bgit\//i.test(ua)) {
      newRequestHeaders.set('User-Agent', 'git/2.34.1');
    }
    // 保持 Authorization（如果有），并确保不误删 Git/LFS 相关头
    newRequestHeaders.delete('x-amz-content-sha256');
    newRequestHeaders.delete('x-amz-date');
  }
  ...
  try {
    ...
    // 递归处理所有 3xx 重定向（适用于 Docker/S3/Git），确保客户端不会直接访问被墙的 CDN
    let redirects = 0;
    while ([301, 302, 303, 307, 308].includes(response.status) && redirects < MAX_REDIRECTS) {
      const redirectUrl = response.headers.get('Location');
      if (!redirectUrl) break;
      console.log(`Redirect #${redirects + 1} -> ${redirectUrl}`);
      redirects++;

      const followHeaders = new Headers(request.headers);
      // 为新的目标设置 Host
      try {
        followHeaders.set('Host', new URL(redirectUrl).hostname);
      } catch (e) {
        console.log('Invalid redirect URL, aborting follow');
        break;
      }

      // 如果重定向指向 S3，添加必要的 AWS 头
      if (isAmazonS3(redirectUrl)) {
        followHeaders.set('x-amz-content-sha256', getEmptyBodySHA256());
        followHeaders.set('x-amz-date', new Date().toISOString().replace(/[-:T]/g, '').slice(0, -5) + 'Z');
      }

      // 保持 Authorization，如果响应提供了新的 Authorization，也保留
      if (response.headers.get('Authorization')) {
        followHeaders.set('Authorization', response.headers.get('Authorization'));
      }

      // 对于 Git 请求，确保 User-Agent 保持 git 客户端风格
      if (isGitRequest) {
        const ua = followHeaders.get('user-agent') || '';
        if (!/\bgit\//i.test(ua)) followHeaders.set('User-Agent', 'git/2.34.1');
      }

      const followReq = new Request(redirectUrl, {
        method: request.method,
        headers: followHeaders,
        body: request.body,
        redirect: 'manual'
      });
      response = await fetch(followReq);
      console.log(`Follow redirect response: ${response.status} ${response.statusText}`);
      // 如果最终请求失败，返回失败的响应
      if (response.status >= 400) {
        console.log('Follow redirect resulted in error, returning that response');
